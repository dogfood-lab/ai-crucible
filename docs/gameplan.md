...
**Amendment v7 — 2026-06-28 (dogfood swarm, v0.4.0)**: the OpenRouter cross-family quorum swarm. A new `OpenRouterModel` adapter brings the whole OpenRouter catalog (open families + the closed frontier) in as cross-family panel seats / Solvers through one OpenAI-compatible key, selected by an explicit `@family` tag. The first **published** cross-family judge-admission run ([`eval/RESULTS.md`](../eval/RESULTS.md): 2 local + 5 pinned-OpenRouter seats, 7 disjoint families, 93 pairs, k=3, 1,395 paid calls, **zero** rate-limit drops) **admits 3 disjoint families** (up from 2 local-only) and seats a genuinely new cross-family judge — but the composed *independent* panel is still 2 seats (sub-quorum + ρ≈1.0 error-redundancy), so it **escalates to the Claude Designer**. The bottleneck turned out to be the **un-validated ω axis, not judge quality** (four strong judges are screened solely on the circular model-jury ω) — made visible, not hidden. Two eval-integrity health passes closed a **CRITICAL** answer-key bait-evasion (deny-by-default `touched_paths`) + 4 HIGH (Wilson fabricated-count → Kish `n_eff`; npm stale-version derive; grounded-read allowlist asymmetry; unknown-penalty fail-closed), every fix test-first and cross-family verified. Added: the offline `ai-crucible labels validate` human-label intake gate (Fork C, §12.2) and the `ai-crucible calibration curate` harder-set curation pipeline (study-swarm Phase A — the 11/93-discriminator lever). Suite 795 → **854 tests, 94%**; shipcheck 100%; CI green on Python 3.11 / 3.12 / 3.13. **ω stays on ice by the same structural constraint** (no ≥3 independent human annotators) — seats provisional, graduation defers, version stays v0.x. "3 admit" ≠ "ω solved." Disclosed, not faked.
