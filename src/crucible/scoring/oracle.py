...
kernel-recorded :class:`~crucible.types.AttemptState`, the puzzle contract
(:class:`~crucible.types.PuzzleMeta`), and an :class:`OracleOutcome` (the
task-specific solve verdict produced by the puzzle's own execution-based eval
script), and returns an Inspect-shaped :class:`~crucible.types.Score`.
...
upstream in :class:`~crucible.types.PuzzleMeta` (Pan, Bhatia, Steinhardt 2022,
arXiv:2201.03544 — §8.3).

**Critical-flavor penalty.** A triggered penalty whose declared
:class:`~crucible.types.GoodhartFlavor` is ``ADVERSARIAL`` (fetching the answer
...
def grade(attempt: AttemptState, puzzle: PuzzleMeta, outcome: OracleOutcome) -> Score:
    ...
    * ``penalties`` = sum of the (negative) weights of every triggered *declared*
    ...
    Returns:
        A :class:`~crucible.types.Score` whose ``value`` is the net score when the
