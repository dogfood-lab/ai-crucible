    ...
    *,
    ...
    Args:
        ...
        model_fn: the objective. Receives an array of shape ``(d, m)`` (d =
            ...
            crucible this is a surrogate of "gate-clear rate on calibration as a
...
class ThresholdoutBudget:
    ...
    crucible's actual Thresholdout noise mechanism (add Laplace noise, only
...
def bo_search(*_args, **_kwargs):  # noqa: ANN002, ANN003, ANN201
    ...
    - Return the proposed weight vector + the trace; the caller compiles it into
      a :class:`~crucible.instrument.rubric_bundle.RubricBundle` and validates it
...
def paraphrase_ablate(*_args, **_kwargs):  # noqa: ANN002, ANN003, ANN201
    ...
    - **Any finding whose sign flips under paraphrase is below crucible's
