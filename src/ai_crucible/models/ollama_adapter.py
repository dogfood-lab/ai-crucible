...
* a cross-family **panel judge** — :meth:`OllamaModel.judge` (+ :meth:`as_judge`,
  which carries the ``.family`` attribute :class:`~ai_crucible.scoring.judge_panel.JudgePanel`
...
* a **characterization** probe — :meth:`OllamaModel.judge_item`, which returns a
  :class:`~ai_crucible.characterize.types.JudgmentRecord` (the §11.1 admission-test
  input unit).
...
class OllamaModel:
    ...
    async def generate(self, state: AttemptState) -> str:
        ...
        Matches :data:`ai_crucible.roles.GenerateFn` (``(AttemptState) -> Awaitable[str]``)
    ...
    def as_judge(self) -> JudgeFn:
        ...
        :class:`~ai_crucible.scoring.judge_panel.JudgePanel` reads the model family off a
